#!/usr/bin/env bash
# Run paper-style WorldCoder baselines: offline RQ1/RQ2 and shared-planner RQ3.
...
WORLDCODER_OUT_DIR="${WORLDCODER_OUT_DIR:-${PROGRAMMATIC_ROOT}/worldcoder}"
RQ3_OUT_DIR="${RQ3_OUT_DIR:-${PATCHWORLD_BASELINE_ARTIFACTS_ROOT}/results/rq3/worldcoder}"
...
run_rq12() {
  mkdir -p "${WORLDCODER_OUT_DIR}"
  cd "${ABDUCTWORLD_REPO}"
  echo "[worldcoder] RQ1/RQ2 output=${WORLDCODER_OUT_DIR}"
  python -u -m test_scripts.programmatic.run_programmatic_rq12 \
    --method worldcoder_v2 \
...
run_rq3() {
  ...
  local log_file="${RQ3_LOG_DIR}/worldcoder_${stamp}.log"
  echo "[worldcoder] RQ3 output=${RQ3_OUT_DIR}"
  echo "[worldcoder] RQ3 log=${log_file}"
...
echo "[worldcoder] data=${DATA_ROOT}"
echo "[worldcoder] poe_world_repo=${POE_WORLD_REPO}"
